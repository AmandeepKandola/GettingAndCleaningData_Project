...
## STEP 2: Merges the training and the test data sets to create one data set 
...
# add column name subjectID for subject files 
names(subject_train) <- "subjectID" 
names(subject_test) <- "subjectID" 

# add column names for featureNames
...
# combine subject train, y_train, x_train etc.. into one dataset 
train <- cbind(subject_train, y_train, X_train) 
test <- cbind(subject_test, y_test, X_test) 
combined <- rbind(train, test) 

## STEP 3: get the measurements on the mean and standard deviation for each measurement. 

# check  which columns contain "mean()" or "std()" 
...
## STEP 4: Uses descriptive activity names to explain the name the activities in the data set. 
